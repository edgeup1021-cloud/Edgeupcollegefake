...
import CharacterModel from "@/components/student/mock-interview/CharacterModel";
import SpeechBubble from "@/components/student/mock-interview/SpeechBubble";
...
export default function MockInterviewPage() {
  ...
  const pendingAssessmentRef = useRef<InterviewAssessment | null>(null);
  ...
  const handleUserInput = useCallback(async (input: string, disableTools = false) => {
    ...
    setState("processing");
    setShowBubble(false); // Hide previous bubble when user sends reply
    ...
    try {
      ...
      if (response.tool_call) {
        if (response.tool_call.name === "show_coding_challenge" && response.tool_call.challenge) {
        ...
        } else if (response.tool_call.name === "end_interview" && response.tool_call.assessment) {
          console.log("=== FRONTEND: Storing pending assessment ===");
          console.log("Assessment:", response.tool_call.assessment);
          pendingAssessmentRef.current = response.tool_call.assessment;
        }
      }
  ...
  const generateReport = useCallback(async (assessment: InterviewAssessment) => {
    ...
    try {
      const response = await fetch("/api/generate-report", {
        ...
        body: JSON.stringify({
          ...
          startTime: interviewStartTimeRef.current,
        }),
      });
      ...
      const report: InterviewReport = await response.json();
      console.log("=== FRONTEND: Report generated ===", report);
  ...
  const handleAudioEnd = useCallback(() => {
    ...
    console.log("Pending assessment:", pendingAssessmentRef.current);

    // Check if interview has ended
    if (pendingAssessmentRef.current) {
      console.log("=== FRONTEND: Interview ended, generating report ===");
      setShowBubble(false); // Hide bubble when interview ends
      const assessment = pendingAssessmentRef.current;
      pendingAssessmentRef.current = null;
      generateReport(assessment);
      setState("idle"); // Interview complete
    } else if (pendingLanguagePromptRef.current) {
      ...
      setState("listening"); // Keep bubble visible while showing buttons
      ...
      setShowBubble(false); // Hide bubble when entering coding mode
      ...
      setShowBubble(false); // Hide bubble when returning to coding
      setState("coding");
    } else {
      // Normal Q&A - keep bubble visible until user replies
      setState("listening");
    }
  }, [startChallenge, codingState.isActive, generateReport]);
        ...
        {isGeneratingReport && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <div className="bg-white rounded-2xl shadow-xl p-8 flex flex-col items-center gap-4">
              ...
              <p className="text-slate-700 font-medium">Generating your interview report...</p>
